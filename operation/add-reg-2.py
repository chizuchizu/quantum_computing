...
class Adder(cirq.ArithmeticOperation):
    ...
    def __init__(self, target_register, input_register, work_registers):
        self.input_register = input_register
        self.target_register = target_register
        self.work_registers = work_registers
    ...
    def apply(self, input_value, target_value, work_registers):
        # print(target_value, input_value, target_value + input_value)
        return target_value + input_value + work_registers
    ...
    def registers(self):
        ...
        # 最初に渡すbit列に書き込まれる
        # #の番号順になっている
        return self.work_registers, self.input_register, self.target_register


"""Example of using an Adder in a circuit."""
# Two qubit registers.
qreg1 = cirq.LineQubit.range(2)
qreg2 = cirq.LineQubit.range(2, 6)
qreg3 = cirq.LineQubit.range(6, 8)

# Define the circuit.
circ = cirq.Circuit(
    #  cirq.ops.X.on(qreg1[1]),
    # cirq.ops.X.on(qreg3[0]),
    cirq.ops.X.on_each(qreg3),
    Adder(input_register=qreg1, target_register=qreg2, work_registers=qreg3),
    cirq.measure_each(*qreg1),
    cirq.measure_each(*qreg2),
    cirq.measure_each(*qreg3)
)
